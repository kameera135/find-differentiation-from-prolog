...
start :-
   ...
   write('Algebraic Simplification.'), nl,
   symdiff1.
...
d( C, _, 0 ):- atomic(C).          /* If C is a constant then */
                                   /* d(C)/dX is 0            */
...
simp(A+B, C):- numeric(A),numeric(-B),C is A-(-B).
...
simp( _*0, 0 ).         /* multiples of zero are zero */

simp( 0*_, 0 ).         /* multiples of zero are zero */

simp( 0/_, 0 ).         /* numerators evaluating to zero are zero */
...
simp(A*(B*X),Z):- A\=X, B\=X, simp((A*B)*X,Z).
simp(A*X^B*C ,Z) :- A\=X, B\=X, C\=X, simp((A*C)*X^B,Z).
simp((X^A)^B,Z):- A\=X, B\=X, simp(X^(A*B),Z).
simp(X^B/X^C,Z):- B\=X, C\=X, simp(X^(B-C),Z).
simp(2*X/X^2*X^2,Z):- 2\=X,simp(2/X^3,Z).
